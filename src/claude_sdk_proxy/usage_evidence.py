...
class DialectUsageMapping:
    ...
    def __post_init__(self) -> None:
        ...
        expected = _digest(b"claude-sdk-proxy:usage-mapping:v1", projection)
...
class UsageEvidenceRow:
    ...
    def __post_init__(self) -> None:
        ...
        expected = _digest(
            b"claude-sdk-proxy:usage-row:v1",
            _row_projection(
                self.key,
                self.fields,
    ...
    def from_json(cls, value: object) -> UsageEvidenceRow:
        ...
        expected = _digest(b"claude-sdk-proxy:usage-row:v1", projection)
        return cls(
            key=key,
            fields=fields,
            sdk_shape_passed=sdk_shape_passed,
...
class UsageEvidenceSchema:
    ...
    def __post_init__(self) -> None:
        ...
        expected = _digest(
            b"claude-sdk-proxy:usage-schema:v1", _schema_projection(self.rows)
        )
    ...
    def from_json(cls, value: object) -> UsageEvidenceSchema:
        ...
        expected = _digest(
            b"claude-sdk-proxy:usage-schema:v1", _schema_projection(rows)
        )
        return cls(
            schema_version=1,
            rows=rows,
