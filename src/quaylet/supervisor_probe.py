...
from quaylet.environment import EnvironmentConfig
...
from quaylet.lifecycle import ProcessIdentity, Record
...
def _supervisor_path() -> Path:
    path = _repository_root() / "build/bin/quaylet-supervisor"
...
def _probe_supervisor_path() -> Path:
    path = _repository_root() / "build/bin/quaylet-supervisor-probe"
    if not path.is_file() or not os.access(path, os.X_OK):
...
def _probe_child_path() -> Path:
    path = _repository_root() / "build/bin/quaylet-probe-child"
    if not path.is_file() or not os.access(path, os.X_OK):
...
def _teardown_recorded_fresh_group(pgid: int) -> bool:
    ...
    allowed = {
        ...
        str(_repository_root() / "build/bin/quaylet-anchor"),
        "/bin/sleep",
        "sleep",
    }
