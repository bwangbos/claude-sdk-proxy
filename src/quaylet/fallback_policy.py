...
from quaylet.model_catalog import backend_model, canonical_model
...
def resolve_fallback(values: list[str], default: RefusalFallback) -> RefusalFallback:
    if len(values) > 1:
        raise RequestValidationError(
            "X-Quaylet-Refusal-Fallback", "must not be repeated"
        )
    value = default if not values else values[0]
    if value not in {"off", "auto"}:
        raise RequestValidationError(
            "X-Quaylet-Refusal-Fallback", "must be exactly off or auto"
        )
