...
from quaylet.asgi_stream import (
    ClosableEventStream,
    DisconnectMonitor,
    EventStreamResponse,
    abort_best_effort,
...
from quaylet.diagnostics import DiagnosticContextMiddleware
...
from quaylet.fallback_policy import (
    RefusalFallback,
    native_allowlist,
    resolve_fallback,
)
from quaylet.http_errors import error_detail, error_response
from quaylet.model_catalog import canonical_models
...
from quaylet.openai_subscription.backend import Backend as SubscriptionBackend
from quaylet.openai_subscription.translation import MODELS as OPENAI_MODELS
...
async def _handle(request: Request, *, dialect: str) -> Response:
    ...
    try:
        ...
        policy = resolve_fallback(
            request.headers.getlist("x-quaylet-refusal-fallback"),
            "off"
        ...
        if parsed.model in OPENAI_MODELS:
            ...
            lease = await registry.open_turn(
                parsed, request.headers.get("x-quaylet-session")
            )
...
def _identity_headers(lease: TurnLeaseProtocol, identity: ResponseIdentity) -> None:
    lease.response_headers["X-Quaylet-Requested-Model"] = identity.requested_model
    if identity.verified:
        lease.response_headers["X-Quaylet-Actual-Model"] = identity.actual_model
    else:
        lease.response_headers.pop("X-Quaylet-Actual-Model", None)
    if identity.fallback:
        lease.response_headers["X-Quaylet-Fallback"] = "true"
